...
# volume per voxel information (needed to compute parcel volume)
voxel_spacing=(`mrinfo ${parc} -spacing`)

voxel_volume=`awk -v v1=${voxel_spacing[0]} -v v2=${voxel_spacing[1]} -v v3=${voxel_spacing[2]} 'BEGIN{total=(v1*v2*v3); print total;}'`

# compute number of voxels per parcel
parcel_mass=(`labelstats ${parc} -output mass`)

# compute per parcel volume
if [ ! -f parcel_volume.txt ]; then
    
    for (( i=0; i<${#parcel_mass[*]}; i++ ))
    do
        volume=`awk -v v1=${voxel_volume} -v v2=${parcel_mass[$i]} 'BEGIN{total=(v1*v2); print total;}'`
        echo $volume >> parcel_volume.txt
    done
fi
...
do
    ...
    if [ ! -f ./assignments/${bname}_assignments.csv ]; then
        ...
        sed 1,1d ./assignments/${bname}_assignments.csv > tmp.csv
        cat tmp.csv > ./assignments/${bname}_assignments.csv
        rm -rf tmp.csv
    fi
done
